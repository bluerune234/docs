...
Tasks are autonomous background workflows that Wovly executes over time. Unlike chat (immediate responses), Tasks handle multi-step processes that may require waiting for replies, coordinating schedules, monitoring for events, or following up over days.
...
| `check_time_passed` | Check if current time >= target |
...
| Tool | Description |
|------|-------------|
| `evaluate_condition` | Compare values (==, !=, >, <, contains) |
